...
function renderError(response, error) {
    ...
    if (error.code === 'ER_BAD_DB_ERROR') {
        return response.status(500).send('La base de datos lab17_marvel no existe. Importa primero el archivo lab17.sql en phpMyAdmin o MySQL.');
    }
...
exports.getEditar = (request, response, next) => {
    if (!request.session.accesoPermitido) {
        request.session.mensajeAcceso = `
            <div class="alert alert-warning">
                Debes validar tu acceso seguro para editar personajes.
            </div>
        `;
        return response.redirect('/acceso');
    }
...
exports.postGuardarPassword = (request, response, next) => {
    ...
    if (!passwordValida) {
        ...
        request.session.mensajeAcceso = `
            <div class="alert alert-danger">
                Contrasena invalida. Debe tener al menos 8 caracteres, una mayuscula, un numero y coincidir en ambos campos.
            </div>
        `;
        return response.redirect('/acceso');
    }
    ...
    request.session.mensajeAcceso = `
        <div class="alert alert-success">
            Contrasena validada correctamente. La sesion quedo activa.
        </div>
    `;
