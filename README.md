# Software Development Labs
### José Eduardo Viveros Escamilla - A01710605


# Laboratorios del curso

| Laboratorio | Tema |
|-------------|------|
| Lab 01 | Introducción a las aplicaciones web, HTML5 y ciclo de vida de los sistemas de información |
| Lab 02 | Control de versiones |
| Lab 03 | CSS |
| Lab 04 | Fundamentos de JavaScript |
| Lab 05 | Frameworks de estilo |
| Lab 06 | Programación orientada a eventos |
| Lab 07 | Manejo de ramas |
| Lab 08 | Introducción al back-end |
| Lab 09 | DBMS de escritorio (Ricardo) |
| Lab 10 | Rutas y formas |
| Lab 11 | Express |
| Lab 12 | — |
| Lab 13 | — |
| Lab 14 | — |
| Lab 15 | — |
| Lab 16 | — |
| Lab 17 | — |
| Lab 18 | — |
| Lab 19 | — |